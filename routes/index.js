...
router.get("/other-profile/:id", (req, res) => {
  try {
    User.findById(req.params.id)
      .populate("lists")
      .then((user) => {
        res.render("otherPeopleProfile", {
          title: user.name,
          user:user,
          lists: user.lists,
        });
...
router.get("/delete-blog/:id", (req, res, nex) => {
...
// exp
router.get("/read-blog/:id", isLoggedIn, (req, res, next) => {
  try {
  ...
  User.findOne({ email: req.body.email })
    .then((user) => {
      ...
      const transport = nodemailer.createTransport({
        ...
        port: 465,
        auth: {
          user: "computerlogic25@gmail.com",
          pass: "aqsnwvvtlqejnomg",
        },
      });
...
router.get("/write", isLoggedIn, function (req, res, next) {
  res.render("write", { title: "Write Blog", user: req.user });
});
